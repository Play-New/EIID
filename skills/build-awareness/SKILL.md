...
5. **Interface signal:** if this code serves a human-facing node, does the implementation capture signal from the interaction? What the user accepts, ignores, modifies, or requests should be observable. A Delivery node that sends alerts but does not track responses is a surface that does not learn — it cannot contribute to the World Model. Flag the gap.
