...
**Approach:** Automate via buy — receipt OCR and ingredient detection are commodity services. No competitive advantage in building custom image processing.
...
**Approach:** Automate via workflow — fixed sequence: parse receipt → extract ingredients → score against recipe DB. Graduation: move to code when recipe DB > 10k and matching patterns stabilize.
...
**Approach:** Differentiate via agent — conversational adaptation requires iteration. "Make it spicier", "I don't have yogurt, what can I substitute?" — the agent loops with tools until the recipe fits. Flexibility is the feature.
...
**Approach:** Innovate via agent — the agent IS the product. WhatsApp-native recipe experience: intake, processing, suggestion, follow-up in one conversation. No graduation target. Code only for the web archive (read-only, deterministic).

## Technology Constraints
Use Supabase for database and auth, NOT Firebase, AWS Amplify.
Use Resend for email, NOT SendGrid, Mailgun.
...
- **Parity:** every user action in web UI (save recipe, edit collection, update preferences) also available as agent tool
- **Granularity:** tools are primitives — get_recipe, search_pantry, update_preference — not compound actions
- **Tests as reward signal:** recipe matching tests define correct output, agent iterates until pass
- **External state:** conversation progress and user preferences persist to Supabase, not conversation history
